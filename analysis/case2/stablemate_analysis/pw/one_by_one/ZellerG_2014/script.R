...
envs[envs != "ZellerG_2014"] <- 'others'

Y <- as.numeric(factor(data_list[[3]]$sample_type))-1
X <- data_list[[1]]

# Define a special objective function, so that the predictivity of predictors are measured within the ZellerG_2014 cohort.
obj_BIC_logit <- function(Y, X, env, var_in, sample_id = 1:length(Y)){
  
  sample_id <- which(env == "ZellerG_2014") # Only consider samples within the ZellerG_2014 cohort.
...
mod_list <- foreach(i=1:niter, .verbose = T) %dopar% {
  ...
  library(arm)
  library(Matrix)
  mod <- SRST2E(Y, X, envs, K = K, max_size = sum(envs == "ZellerG_2014"),
...
save.image('./case2/r_object/pw/one_by_one/ZellerG_2014.RData')
