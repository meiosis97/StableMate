...
envs[envs != "FengQ_2015"] <- 'others'

Y <- as.numeric(factor(data_list[[3]]$sample_type))-1
X <- data_list[[1]]

# Define a special objective function, so that the predictivity of predictors are measured within the FengQ_2015 cohort.
obj_BIC_logit <- function(Y, X, env, var_in, sample_id = 1:length(Y)){
  
  sample_id <- which(env == "FengQ_2015") # Only consider samples within the FengQ_2015 cohort.
...
mod_list <- foreach(i=1:niter, .verbose = T) %dopar% {
  ...
  library(arm)
  library(Matrix)
  mod <- SRST2E(Y, X, envs, K = K, max_size = sum(envs == "FengQ_2015"),
...
save.image('./case2/r_object/pw/one_by_one/FengQ_2015.RData')
