...
mod_list <- foreach(i=1:niter, .verbose = T) %dopar% {
  ...
  while(length(idxSb)==0 | length(idxSb) == length(idxMb)){
    {  nNode <- 200 # Number of variables
    propE <- 0.025 # Proportion of edges being connected
    nI <- 120 # Number of intervention
    ...
    idxY <- nI + 100 # Set the id of the response
  ...
  time_record[1, 'SR'] <- system.time({
    ...
    mod_sr <- SRanalysis(trainX, trainY, group, num_reps = 100, prescreen_types = 'lasso', pred_scores = 'bic',
                         pars = list(stab_test  = 'exact', B = 4000, prescreen_size = 40,
...
save.image('./simulation/r_object/sb_200.RData')
