...
``` r
#' @param pred_st2_ctrl Controls for st2e function that selects most predictive features of the response.
#' pred_pool: A vector or a matrix that restricts the variable selection space. If a vector is given, it must
#' be a vector of characters that is a subset of the column names of X. Section of "pseudo_predictor" can be enabled by adding a "pseudo_predictor" into pred_pool.
#' When a matrix is given, it must be an indicator matrix
