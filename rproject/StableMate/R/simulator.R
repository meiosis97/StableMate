...
# Exported
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#' Simulate a directed asymmetric graph.
#'
#' Simulate a directed asymmetric graph that satisfies the model assumption of
#' stabilized regression (\href{https://doi.org/10.1214/21-AOAS1487}{Pfister et al. (2019, The Annals of Applied Statistics)}).
#'
#' @param nSample Numerical; Number of samples to simulate per environments.
#' @param nE Numerical; Number of environments to simulate.
#' @param nNode Numerical; Number of nodes (one response + predictors) in the system.
#' @param propE Numerical; Proportion of edges to be connected.
#' @param nI Numerical; A number between 1 and \code{nNode} controls the number of predictors to be intervened.
#' @param idXY Numerical; A number between 1 and \code{nNode} represents a variable index to be chosen as the response
#'
#' @return A \code{list} contains simulation results.
#'
#' @export
#'
#' @rdname sim_scm
sim_scm <- function(nSample = 300, nE = 4, nNode = 50, propE = 0.1, nI = 30, idxY = 25){
  ...
  while(length(idxSb)==0 | length(idxSb) == length(idxMb)){
    ...
    group <- factor(rep(paste("e",1:(nE-1), sep = ""),each =nSample)) # Environment variable
