...
DPT <- readRDS('/data/gpfs/projects/punim1662/yidi_projects/gbm/data/dpt.rds') # Load the diffusion pseudo time object
...
Y <- DPT$DPT366 # The cell 366 was selected as the root of the cell state transition
X <- as.matrix(t(impdata))
Y_sub <- Y[envs == 'Core']
X_sub <- X[envs == 'Core',]

# Define a special objective function, so that the predictivity of predictors are measured within the core
obj_BIC <- function(Y, X, env, var_in, sample_id){
  
  sample_id = which(env == 'Core') # Only consider samples within the core
...
save.image('./case3/r_object/dpt_gene/core.RData')